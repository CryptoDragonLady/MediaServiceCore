...
internal enum class AppClient(
    ...
    WEB_EMBED(CLIENTS.WEB_EMBEDDED.NAME, CLIENTS.WEB_EMBEDDED.VERSION, CLIENT_NAME_IDS[CLIENTS.WEB_EMBEDDED.NAME],
        userAgent = DefaultHeaders.USER_AGENT_WEB, referer = CLIENTS.WEB_EMBEDDED.REFERER, clientScreen = CLIENT_SCREEN_EMBED),
    ...
    val isWebPoTokenSupported by lazy {
        clientName in setOf(
            CLIENTS.WEB.NAME,
            CLIENTS.MWEB.NAME,
            CLIENTS.TV.NAME,
            CLIENTS.WEB_EMBEDDED.NAME,
            CLIENTS.WEB_CREATOR.NAME,
            CLIENTS.YTMUSIC.NAME,
            CLIENTS.TV_SIMPLY.NAME,
            CLIENTS.TV_EMBEDDED.NAME
        )
    }
    val isPlayerPoTokenRequired by lazy { Helpers.equalsAny(this, WEB, MWEB, WEB_EMBED, WEB_SAFARI, INITIAL, GEO) }
